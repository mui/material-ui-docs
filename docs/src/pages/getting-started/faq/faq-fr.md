...
La raison n°1 est probablement due à des conflits de noms de classe une fois que votre code est dans un bundle de production. Pour que Material-UI fonctionne, les valeurs `className` de tous les composants d'une page doivent être générées par une seule instance du [générateur de noms de classe](/styles/advanced/#class-names).
